...
const Notification = () => {
    ...
    useEffect(() => {
        socket.on('getNofication', (data: NotificationType) => {
    ...
    return (
        <div className="relative">
            ...
            >
                <div className="relative">
                    <Image path={"icons/notification.svg"} h={24} w={24} alt=''/>
           ...
           {open && <div className="absolute -right-full p-4 rounded-lg bg-white text-black flex flex-col gap-4 w-max">
                ...
                <button onClick={reset} className="bg-black text-white text-sm rounded-lg">Mark as read</button>
            </div>}
        </div>
    )
}
