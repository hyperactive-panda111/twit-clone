...
export const ImageEditor = ({ onClose, previewURL, settings, setSettings }: ImageEditorProps) => {

	const handleChangeSensitive = (sensitive: boolean) => {
		setSettings(prev => ({ ...prev, sensitive }));
	}

	const handeChangeType = (type: 'original' | 'wide' | 'square') => {
		setSettings(prev => ({...prev, type }));
	}

	return (
		<div className="fixed bg-black bg-opacity-75 h-screen w-screen top-0 left-0 z-10 flex items-center justify-center">
			<div className="bg-black rounded-xl p-12 flex flex-col gap-4">
				...
				<div className="flex justify-between items-center">
					...
					<button className="py-2 px-4 rounded-full bg-white text-black font-black" onClick={onClose}>Save</button>
				</div>
				{/* IMAGE CONTAINER */}
				...
				<div className="flex items-center justify-between text-sm">
					...
					<div className="flex items-center cursor-pointer gap-4" onClick={() => handeChangeType('original')}>
						...
						<div className="flex items-center cursor-pointer gap-4" onClick={() => handeChangeType('square')}>
							<svg width={24} viewBox="0 0 24 24">
								<path
						...
						<div className="flex items-center cursor-pointer gap-4" onClick={() => handeChangeType('wide')}>
							<svg width={24} viewBox="0 0 24 24">
								<path
